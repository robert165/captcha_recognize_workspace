...
#	liu,yu 2018-05-22 
#	V 1.0.0
#描述：部署工程captcha_recognize的docker和添加volume，步骤如下：
#1.地址：显示当前IP等信息，询问是否更改(未完成)
#2.用户：是否切换或创建用户，是则创建并切换到新用户（未完成）
#3.询问安装cpu模式或gpu模式的docker
#4.输入volume本地地址
#5.安装docker，gpu需要安装nvidia-docker
#6.编译.../captcha_recognize/docker/cpuMy/Dockerfile.captcha_recognize.cpu或.../captcha_recognize/docker/gpu/Dockerfile.captcha_recognize.gpu
#7.启动镜像，并加载volume
#
#   ../captcha_recognize_workspace 目录结构
#	|--data	#模型
#	|--captcha_recognize #代码
#	|--img	#检测数据
#
# 注意：
...
echo "# 1.cd captcha_recognize/"
echo "# 2./workspace/captcha_recognize/bin/deep_ocr_id_card_reco --img /workspace/img/xuanye.jpeg             --debug_path /tmp/debug             --cls_sim /workspace/data/chongdata_caffe_cn_sim_digits_64_64             --cls_ua /workspace/data/chongdata_train_ualpha_digits_64_64
"

IMAGENAME=captcha_recognize_dockerimage
...
do
	...
	echo "For Example:/home/$USER/DataCenterProject/captcha_recognize_workspace "
...
echo "############################################################################"
echo "##									##"
echo "##	检测是否有captcha_recognize_DockerImage:py27_cpu或captcha_recognize_DockerImage:py27_gpu镜像	##"
echo "##	编译.../captcha_recognize/docker/cpuMy/Dockerfile.captcha_recognize.cpu		##"
echo "##	或.../captcha_recognize/docker/gpu/Dockerfile.CHINESE-OCR.gpu		##"
echo "##									##"
echo "############################################################################"
...
then
	...
	then
		#sudo docker build -t $IMAGENAME:$IMAGETAGCPU -f ${VOLUMEPATH}/Dockerfile.captcha_recognize.cpu ${VOLUMEPATH}
		sudo docker build -t $IMAGENAME:$IMAGETAGCPU -f ${VOLUMEPATH}/docker/cpu/Dockerfile.captcha_recognize.cpu ${VOLUMEPATH}/docker/cpu
	else 
...
then
	...
	then
		#sudo docker build -t $IMAGENAME:$IMAGETAGGPU -f ${VOLUMEPATH}/Dockerfile.captcha_recognize.gpu ${VOLUMEPATH}
		sudo docker build -t $IMAGENAME:$IMAGETAGGPU -f ${VOLUMEPATH}/docker/gpu/Dockerfile.captcha_recognize.gpu ${VOLUMEPATH}/docker/gpu
	else 
